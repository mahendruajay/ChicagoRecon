...
public class DestinationSuggestionService {
    ...
    private Map<String, Destination> determineNextDestination(String userId, String origin){
        ...
        if(user != null && (user.getLiked().keySet().size() > 0 || user.getDisliked().keySet().size() > 0)){
            ...
            if(alreadyVisited.size() == destinations.getDestinations().size()){
                ...
                log.info("");
                log.info("ALL DESTINATIONS VIEWED - RESTARTING...");
                log.info("");
                if(liked.size() > 0) {
                    //TODO: Should try to prefer showing liked if liked destinations exist once all shown???
                    user.getLiked().clear();
                    return getRandomStartAndAccurateNexts();
                } else {
                    user.getDisliked().clear();
                    return getRandomStartAndAccurateNexts();
                }
            }
    ...
    private Map<String, Destination> getRandomStartAndAccurateNexts(){
        Random rn = new Random();
        Map<String, Destination> returnVal = new HashMap<>();
